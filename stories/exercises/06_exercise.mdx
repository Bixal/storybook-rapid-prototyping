...
1. Visit the [banner component](https://designsystem.digital.gov/components/banner/) page. Find the "Banner settings" section.
...
1. Easy, right? Let's add a bit more color to our banner. Let's see what would happen if we brought back our accent color:
    ...
    If the automated checks determine neither color meets contrast requirements, we'll allow you to override but give you a warning. If we inspect this warning, we learn that the link color and it's fallback do not meet color contrast minimums.
