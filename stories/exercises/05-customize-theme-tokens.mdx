...
2. Open `uswds-theme.scss`. Uncomment the `@use 'uswds-core with (...)` section and paste the theme settings from the chosen palette between the parenthesis.
    ```scss
    // uswds-theme.scss
    @use "uswds-core" with (
...
4. There are many diverse theme settings available. For example, lets update our heavy font weight and increase our border radiuses.
    ```scss
