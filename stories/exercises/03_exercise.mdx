...
1. In `args` add the following settings.
   ...
   ```
1. You should now be able to modify the strings in real time.
1. Let's modify the CTA so that it re-uses our existing button. Open `/stories/components/hero/hero.html.twig:27`.
1. Replace the string with a twig include for the button component. Use the `@components` shorthand for easier pathing. _These shorthands were setup in `vite.config.js`_
   ```html
   {% if cta %} {% include "@components/button/button.html.twig" %} {% endif %}
   ```
...
1. We can pass our hero cta data by passing the object in the twig include.
